...
# 		self._algorithm._setup_contants (self)
...
class CosineScoring (Scoring):
	...
	def _docv_len (self, docv, vocabulary, D, term_mapping):
		'''
		...
		TF = 1
		tfs = docv['tf']
		weights = [self._tfidf (D, vocabulary[term_mapping[i[TERMID]]]['df'], i[TF]) for i in tfs]
	...
	def create_scoring_data (self, retrieval, postings_lists, query_tokens):
		...
		term_mapping = {v['termid']:t for t,v in vocabulary.items ()}
		...
		docvs = retrieval.indexing.get_doc_vectors (docids)
		tf_lists = [d[PL_TF] for d in postings_lists]

		for docid, tfs, docv in zip (docids, tf_lists, docvs):
			docv_len = self._docv_len (docv, vocabulary, D, term_mapping)
	...
	def score (self, scoring_data=None):
		'''
		...
		::return:: a list of descending sorted scores, whose structure is [[docid, score], ...]
		'''	
